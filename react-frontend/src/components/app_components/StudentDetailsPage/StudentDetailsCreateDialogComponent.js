...
const StudentDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,address: _entity?.address,image: _entity?.image,
    ...
    return (
        <Dialog header="Create StudentDetails" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
...
<div className="col-12 field">
                    <span className="align-items-center">
                        ...
                        <UploadFilesToS3 type={'create'} user={props.user} id={urlParams.id} serviceName="studentDetails" onUploadComplete={setimageId} onFileLoaded={onFileimageLoaded}/>
                    </span>
                    <small className="p-error">
